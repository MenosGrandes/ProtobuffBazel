...
if len(sys.argv) != 3:
    print("Usage: python draw.py <Benchmark1> <Benchmark2>")
    print("Example: python draw.py NoArena Arena")
    sys.exit(1)
...
# Split datasets based on user input
bench1 = df[df["name"].str.startswith(f"BM_{bench1_str}/")]
bench2 = df[df["name"].str.startswith(f"BM_{bench2_str}/")]
if bench1.empty:
    available = ", ".join(sorted(set(s.split("/")[0][3:] for s in df['name'])))
    raise ValueError(f"No benchmark starting with '{bench1_str}' found in CSV. Available benchmarks: {available}")
if bench2.empty:
    available = ", ".join(sorted(set(s.split("/")[0][3:] for s in df['name'])))
    raise ValueError(f"No benchmark starting with '{bench2_str}' found in CSV. Available benchmarks: {available}")
...
# Compute percentage speedup: (bench1 - bench2) / bench2 * 100
merged['percent_speedup'] = (merged[f'real_time_{bench1_str}'] - merged[f'real_time_{bench2_str}']) / merged[f'real_time_{bench2_str}'] * 100
...
ax1.set_yscale('log')
ax1.set_ylabel('Time')
ax1.set_title(f'Protobuf benchmark: {bench1_str} vs {bench2_str}')
ax1.grid(True, which='both', ls='--', alpha=0.5)
ax1.legend()

# Bottom plot: percentage speedup
